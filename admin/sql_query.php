...
<?php if (isset($_POST['setting'])) {
	include "../koneksi.php";
	$notif_stok		= $_POST['notif_stok'];
	$limit_supplier	= $_POST['limit_supplier'];
	$limit_barang	= $_POST['limit_barang'];
	$limit_user		= $_POST['limit_user'];
	$limit_masuk	= $_POST['limit_masuk'];
	$limit_keluar	= $_POST['limit_keluar'];

	// Cek apakah gambar berhasil diupload atau tidak  // Proses simpan ke Database

	$queryedit = "update setting set notif_stok='" . $notif_stok . "', limit_supplier ='" . $limit_supplier . "', limit_barang ='" . $limit_barang . "',
 limit_user ='" . $limit_user . "', limit_masuk ='" . $limit_masuk . "', limit_keluar ='" . $limit_keluar . "'";
	$sqledit = mysqli_query($conn, $queryedit); // Eksekusi/ Jalankan query dari variabel $query
	if ($sqledit) {
		header("location: setting.php");
	} else {
		echo "Maaf, Terjadi kesalahan saat mencoba untuk menyimpan data ke database.";
		echo "<br><a href='setting.php'>Kembali Ke Form</a>";
	}
}
?>

<?php if (isset($_POST['tambahSupplier'])) {
	include "../koneksi.php";
...
'ada')";
	...
	if ($sql) {
		...
		echo "Maaf, Terjadi kesalahan saat mencoba untuk menyimpan data ke database.";
		echo "<br><a href='supplier.php'>Kembali Ke Form</a>";
	}
}
?>

<?php if (isset($_POST['updateSupplier'])) {
	include "../koneksi.php";
	...
	$queryedit = "update tb_supplier set nm_supplier='" . $nm_supplier . "', alamat ='" . $alamat . "', kota ='" . $kota . "', no_hp ='" . $no_hp . "'
...
<?php if (isset($_POST['deleetSupplier'])) {
	include "../koneksi.php";
	$id_supplier	= $_POST['id_supplier'];

	// Cek apakah gambar berhasil diupload atau tidak  // Proses simpan ke Database

	$queryhapus = "update tb_supplier set ket='tidak' where id_supplier = '" . $id_supplier . "'";
...
<?php if (isset($_POST['tambahBarang'])) {
	...
	// Cek apakah gambar berhasil diupload atau tidak  // Proses simpan ke Database
	$cari = mysqli_query($conn, "select max(id_brg) as id_brg from tb_barang where id_supplier='$id_supplier'");
	$hasil_cari = mysqli_fetch_array($cari);
	$kode = substr($hasil_cari['id_brg'], 3, 4);
	$kode_brg = $kode + 1;
	$masukan = $id_supplier . str_pad($kode_brg, 4, "0", STR_PAD_LEFT);
...
<?php if (isset($_POST['updateBarang'])) {
	...
	// echo '<pre>';
	// var_dump($nm_brg, $id_supplier, $stok, $harga_beli, $harga_jual, $id_brg, $tgl_so);
	// die;

	// Cek apakah gambar berhasil diupload atau tidak  // Proses simpan ke Database
	$query = "INSERT INTO tb_update VALUES('','" . $tgl_so . "','" . $id_brg . "','" . $stok . "','" . $harga_beli . "','" . $harga_jual . "','update')";
	$sql = mysqli_query($conn, $query);
	$queryedit = "update tb_barang set nm_brg='" . $nm_brg . "',  stok ='" . $stok . "', harga_jual ='" . $harga_jual . "', harga_beli ='" . $harga_beli . "' where id_brg = '" . $id_brg . "'";
	$sqledit = mysqli_query($conn, $queryedit); // Eksekusi/ Jalankan query dari variabel $query
	if ($sqledit) {
		header("location: barang.php");
	} else {
...
<?php if (isset($_POST['deleetBarang'])) {
	include "../koneksi.php";
	$nm_brg			= $_POST['nm_brg'];
	$stok			= $_POST['stok'];
	$harga_beli		= $_POST['harga_beli'];
	...
	// Cek apakah gambar berhasil diupload atau tidak  // Proses simpan ke Database
	$query = "INSERT INTO tb_update VALUES(
'',
'" . $tgl_so . "',
'" . $id_brg . "',
'" . $stok . "',
...
'delete')";
	$sql = mysqli_query($conn, $query);
	$queryhapus = "update tb_barang set ket='tidak'
 where id_brg = '" . $id_brg . "'";
	$sqlhapus = mysqli_query($conn, $queryhapus); // Eksekusi/ Jalankan query dari variabel $query
	if ($sqlhapus) {
		header("location: barang.php");
...
<?php if (isset($_POST['updateUsr'])) {
	...
	$queryedit = "update tb_user set password='" . $password . "', nama ='" . $nama . "', tgl_lahir ='" . $tgl_lahir . ", alamat='" . $alamat . "', kota ='" . $kota . "', no_hp ='" . $no_hp . "' 
where username = '" . $username . "'";
	$sqledit = mysqli_query($conn, $queryedit); // Eksekusi/ Jalankan query dari variabel $query
	if ($sqledit) {
		header("location: index.php");
...
<?php if (isset($_POST['deleetUsr'])) {
	...
	$queryhapus = "DELETE FROM `tb_user` WHERE `tb_user`.`username` = '" . $username . "'";
	$sqlhapus = mysqli_query($conn, $queryhapus); // Eksekusi/ Jalankan query dari variabel $query
	if ($sqlhapus) {
		header("location: index.php");
	} else {
